...
function loadWidget(config) {
	...
	(function registerEventListener() {
		...
		document.querySelector("#waifu-tool .fa-times").addEventListener("click", () => {
			...
			showMessage("愿你有一天能與重要的人重逢。", 2000, 11);
		...
		devtools.toString = () => {
			showMessage("哈哈，你打開了控制台，係埋想偷睇我噶秘密呢？", 6000, 9);
		};
		window.addEventListener("copy", () => {
			showMessage("你在复制什么呀？，转载記得要加上出处哦！", 6000, 9);
		});
		window.addEventListener("visibilitychange", () => {
			if (!document.hidden) showMessage("哇，你終於翻來啦～", 6000, 9);
		});
	})();
	...
	(function welcomeMessage() {
		...
		if (location.pathname === "/") { // 如果是主页
			...
			if (now > 5 && now <= 7) text = "早上好！早起的鳥兒有蟲吃，早起的蟲兒...?。";
			else if (now > 7 && now <= 11) text = "上午好！工作还好吗？，不要久坐哦，要起身做下運動嘛！";
			else if (now > 11 && now <= 13) text = "中午了，工作了一個上午，现在是午餐時間！";
			else if (now > 13 && now <= 17) text = "午後很容易犯困呢，今天的運動目標完成了嗎？";
			else if (now > 17 && now <= 19) text = "傍晚了！窗外夕陽的景色很美麗呢，最美不過夕陽紅～";
			else if (now > 19 && now <= 21) text = "晚上好，今天過成點樣呀？";
			else if (now > 21 && now <= 23) text = ["已經這麼晚了呀，早點休息吧，晚安～", "深夜時要愛護眼睛呀！"];
			else text = "还不睡觉吗？身体重要哦！";
			...
			else if (domain === "google") text = `Hello！來自 google搜索 的朋友<br>欢迎阅读<span>「${document.title.split(" - ")[0]}」</span>`;
	...
	var userAction = false,
		userActionTimer,
		messageTimer,
		messageArray = ["好久不見，时间过得真快呢……", "坏蛋！你都不理人家，嚶嚶嚶～", "嗨～快来和我一起玩啊！", "拿小拳拳錘你胸口！"];
	...
	async function loadModel(modelId, modelTexturesId, message) {
		...
		if (useCDN) {
			...
			console.log(`2號老婆 ${modelId}-${modelTexturesId} 加載完成`);
		}
	}
	...
	async function loadRandModel() {
		...
		if (useCDN) {
			...
			let target = randomSelection(modelList.models[modelId]);
			loadlive2d("live2d", `${cdnPath}model/${target}/index.json`);
			showMessage("我这件新衣服好看吗？", 4000, 10);
		} else {
			// 可选 "rand"(随机), "switch"(顺序)
			fetch(`${apiPath}rand_textures/?id=${modelId}-${modelTexturesId}`)
				.then(response => response.json())
				.then(result => {
					if (result.textures.id === 1 && (modelTexturesId === 1 || modelTexturesId === 0)) showMessage("我没有其他衣服啦！", 4000, 10);
					else loadModel(modelId, result.textures.id, "我这件新衣服好看吗？");
				});
		}
	}
